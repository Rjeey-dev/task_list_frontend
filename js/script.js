(function (angular) {
    ...
    app.controller("mainController", function ($scope, $http) {
        ...
        $scope.loadTasks = function () {
            ...
            $http.get(apiUri + '/tasks').then(function success(response) {
        ...
        $scope.startEdit = function (task) {
            $scope.comments = [];
            $http.get(apiUri + '/comments?task_id=' + task.id).then(function success(response) {
        ...
        $scope.addComment = function () {
            $http.post(apiUri + '/comments', {
                text: $scope.createCommentText,
        ...
        $scope.delete = function () {
            $http.delete(apiUri + '/tasks/' + $scope.currentTask.id).then(function success(response) {
        ...
        $scope.changeStatus = function (currentTask, status) {
            $http.patch(apiUri + '/tasks/' + currentTask.id, {status: status}).then(function success(response) {
                    $scope.tasks.forEach((task, key) => {
        ...
        $scope.update = function (i) {
            $http.patch(apiUri + '/tasks/' + $scope.currentTask.id, {
        ...
        $scope.addTask = function () {
            $http.post(apiUri + '/tasks', {'name': $scope.createTodoTaskName}).then(function success(response) {
                    $scope.tasks.push(response.data.details);
                    $scope.syncTasks();
                }, function error(response) {
                    console.log("Error");
